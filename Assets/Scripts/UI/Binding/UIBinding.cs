...
namespace TowerDefense.UI.Binding
{
    ...
    public abstract class UIBinding : MonoBehaviour
    {
        ...
        {
            foreach (BindingInfo b in _bindings) b.Event.RemoveEventHandler(b.Source, b.Handler);
            _bindings.Clear();
        }
        ...
        {
            foreach (string raw in _cachedExpressions)
            {
                ...
                string ctxName = expr.Substring(0, dot).Trim();
                string remainingPath = expr.Substring(dot + 1).Trim();

                // 1. Try registry
                IBindingContext ctx = BindingContextRegistry.Get(ctxName);
                ...
                for (int i = 0; i < parts.Length; i++)
                {
                    ...
                    if (part.Contains("[") && part.Contains("]"))
                    {
                        ...
                        string listPropertyName = listParts[0];
                        int index;
                        ...
                        PropertyInfo listProperty = currentType.GetProperty(listPropertyName);
                        ...
                        object listBinding = listProperty.GetValue(currentObject);
                        if (listBinding == null)
                        {
                        ...
                        PropertyInfo valueProperty = listType.GetProperty("Value");
                        ...
                        object list = valueProperty.GetValue(listBinding);
                        if (list == null)
                        {
                        ...
                        PropertyInfo bindableProperty = itemType.GetProperty(parts[i + 1]);
                        ...
                        object propertyValue = bindableProperty.GetValue(currentObject);
        ...
        {
            int c = _bindings.Count;
            if (c == 0) return;

            // rent a small array
            object[] arr = sPool.Rent(c);
            try
            {
                for (int i = 0; i < c; i++)
                {
                    object bindable = _bindings[i].Getter();
                    if (bindable != null)
                    {
        ...
        protected void PushValue<T>(int bindingIndex, T newValue)
        {
            ...
            BindingInfo info = _bindings[bindingIndex];
            object bindable = info.Prop.GetValue(info.Source);
            if (bindable != null)
            {
                PropertyInfo valueProperty = bindable.GetType().GetProperty("Value");
